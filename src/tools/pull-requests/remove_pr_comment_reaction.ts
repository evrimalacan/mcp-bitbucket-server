...
const schema = z.object({
  ...
  emoticon: z
    ...
    .describe('The emoticon to remove (thumbsup, thumbsdown, heart, thinking_face, laugh)'),
});
...
export const removePrCommentReactionTool = (server: McpServer) => {
  server.registerTool(
    'bitbucket_remove_pr_comment_reaction',
    {
      ...
      description:
        'Remove an emoticon reaction from a pull request comment. Only the user who added the reaction can remove it. Supported emoticons: thumbsup, thumbsdown, heart, thinking_face, laugh.',
