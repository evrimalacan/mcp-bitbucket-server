...
export const getPullRequestDiffTool = (server: McpServer) => {
  server.registerTool(
    'bitbucket_get_pull_request_diff',
    {
      ...
      description:
        'Retrieve diff for a pull request. Omit the path parameter to get the full PR diff, or specify a file path to get diff for a specific file. Use format="text" (default) for raw unified diff format, or format="json" for structured diff with hunks and segments. Use contextLines to control how many unchanged lines are shown around changes.',
      inputSchema: schema.shape,
    },
    async ({ projectKey, repositorySlug, pullRequestId, path, contextLines, whitespace, format }) => {
